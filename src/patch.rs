...
use windows::Win32::System::Memory::{
    VirtualProtect, VirtualQuery, MEMORY_BASIC_INFORMATION, PAGE_PROTECTION_FLAGS, PAGE_NOACCESS, PAGE_EXECUTE_READWRITE, MEM_COMMIT,
};
...
pub unsafe fn assert_byte<T>(addr: *const T, expected: u8) -> Result<()> {
    ...
    if actual != expected {
        ...
        return Err(anyhow!("Expected {expected:#02X} at {uaddr:#08X} but found {actual:#02X}"));
    }
...
impl ByteSearcher {
    ...
    pub fn find_bytes_in_ranges<'a, const N: usize>(patterns: &[&[u8]; N], protection: Option<PAGE_PROTECTION_FLAGS>, ranges: impl Iterator<Item = &'a (*const c_void, *const c_void)>) -> Result<[Option<*const c_void>; N]> {
        ...
        for &(start, end) in ranges {
            ...
            while addr < end {
                ...
                if memory_info.State != MEM_COMMIT || protection.is_some_and(|p| !p.contains(memory_info.Protect)) {
                ...
                let search_region = unsafe { std::slice::from_raw_parts(search_base, memory_info.RegionSize) };
                for (&pattern, address) in patterns.iter().zip(addresses.iter_mut()).filter(|(_, a)| a.is_none()) {
    ...
    pub fn discover_modules(&mut self) -> Result<()> {
        ...
        unsafe { EnumProcessModules(hproc, modules.as_mut_ptr(), size_of_val(&modules) as u32, &mut bytes_needed) }?;

        let num_modules = std::cmp::min(bytes_needed as usize / size_of::<HMODULE>(), modules.len());
        ...
        for &module in &modules[..num_modules] {
            ...
            unsafe {
                GetModuleInformation(hproc, module, &mut module_info, size_of_val(&module_info) as u32)?;
                let base = module_info.lpBaseOfDll as *const c_void;
                self.modules.insert(module_name, (base, base.add(module_info.SizeOfImage as usize)));
            }
        }
    ...
    fn get_module_ranges<'b, 'a: 'b, 'c: 'b>(&'a self, modules: &'b [&'c str]) -> impl Iterator<Item = &'a (*const c_void, *const c_void)> + 'b {
        modules.iter().filter_map(|&module_name| self.modules.get(&module_name.to_lowercase()))
    }

    pub fn find_bytes<const N: usize, const M: usize>(&self, patterns: &[&[u8]; N], protection: Option<PAGE_PROTECTION_FLAGS>, modules: &[&str; M]) -> Result<[Option<*const c_void>; N]> {
        if M > 0 {
            ...
            Self::find_bytes_in_ranges(patterns, protection, [&(0x1000 as *const c_void, usize::MAX as *const c_void)].into_iter())
        }
    }
}